...
package FLLOAT.RuntimeVerification;
 ...
 * for each propositional interpretation to the set of symbols. The FLLOAT.automaton has hence to be constructed
...
public class ExecutableAutomaton {
    ...
    public ExecutableAutomaton(Automaton automaton) {
        if(automaton.initials().size() > 1)
            throw new RuntimeException("The input FLLOAT.automaton has more than one initial state!");

        // Reduction and determinization of the FLLOAT.automaton.
    ...
    public State step(TransitionLabel label) {
        ...
        if (arrivalStates.size() != 1)
            throw new RuntimeException("Something went wrong in performing the transition: either no arrival state or more than one (i.e., nondeterministic FLLOAT.automaton)");
