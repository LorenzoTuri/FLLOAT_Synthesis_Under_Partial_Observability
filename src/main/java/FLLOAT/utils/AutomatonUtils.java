...
package FLLOAT.utils;

import FLLOAT.automaton.EmptyTrace;
import FLLOAT.automaton.PossibleWorldWrap;
import FLLOAT.automaton.QuotedFormulaStateFactory;
import FLLOAT.automaton.QuotedFormulaStateFactory.QuotedFormulaState;
import FLLOAT.automaton.TransitionLabel;
import FLLOAT.evaluations.PropositionLast;
...
public class AutomatonUtils {
    ...
    public static Automaton ldlf2Automaton(LDLfFormula initialFormula, PropositionalSignature ps) {
        ...
        // Automaton initialization: empty FLLOAT.automaton
    ...
    private static Automaton eliminateLastTransitions(Automaton oldAut) {
        ...
        for (Transition<TransitionLabel> oldTran : oldTransitions) {
            ...
            if (oldLabel instanceof EmptyTrace) {
                ...
                // Check if the transitions already exists in the new FLLOAT.automaton
                if(! newAut.delta().contains(newTran)) {
                    try {
                        newAut.addTransition(newTran);
                    } catch (NoSuchStateException e) {
    ...
    /**
     * Only to be used with ProM. Returns the FLLOAT.automaton which transitions contain ONE event only.
     * @param oldAut
     * @return
     */
     ...
     * representation of this FLLOAT.automaton.
     */
    ...
    /*
    It builds the FLLOAT.automaton considering only interpretation with a single variable true (declare assumption).
     */
